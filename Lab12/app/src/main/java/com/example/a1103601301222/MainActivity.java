package com.example.a1103601301222;
...
public class MainActivity extends AppCompatActivity {
    ...
    findViewById(R.id.btn_search).setOnClickListener(v ->{
        ...
        new OkHttpClient().newCall(request).enqueue(new Callback) {
                ...
                if (response.code() == 200) {
                    ...
                    for(int i =0;i<items.length;i++) {
                        items[i] = "\n列車即將進入:"+data.result.results[i].Station+
                                "\n列車行駛目的地:"+data.result.result[i].Destination;
                    }
                    runOnUiThread(() -> {
                        new AlertDialig.Builder(MainActivity.this)
                                .setTitle("台北捷運列車到站站名")
                                .setItems(items,null)
                                .show();
                    });
            ...
            @Override
            protected void onCreate(@NonNull Call call,@NonNull IOException e) {
